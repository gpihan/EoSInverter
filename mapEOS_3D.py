...
def key(iT, ib, iq, Nb=5, Nq=5):
    return  (iT*Nq + iq)*Nb + ib

def ToTilde(E, Nb, Nq): # Input GeV^4, GeV^3
    Ttilde = (12/(19*np.pi**2) * E)**0.25# GeV
    muBtilde = (5 * Nb - Nq)/ Ttilde**2 # GeV
    muQtilde = (-1 * Nb + 2 * Nq)/ Ttilde**2 # GeV
    return Ttilde, muBtilde, muQtilde

def ToEN(Tt, mbt, mqt):
    e = 19*np.pi**2/12 * Tt**4 
    nb = 1/3*mbt*Tt**2
    nq = 1/3*2*mqt*Tt**2
    return np.array([e, nb, nq])
...
def write_header(TILDE_TABLES):
    ...
    return [Btm, Qtm, Ttm, dmubtilde, dmuqtilde, dTtilde, NBtilde, NQtilde, NTtilde]
...
def Newton(EN, guessSol, INTERP, MTP):
    ...
    return status, TMUs_sol[0], TMUs_sol[1], TMUs_sol[2]
...
def binary_search_3d(ed_local, nB_local, nQ_local, INTERP, TMU_TABLES, MTP):
    iteration = 0
    muQ_min = TMU_TABLES["MUQ"]["MIN"]; muQ_max = TMU_TABLES["MUQ"]["MAX"]
    ((T_min, muB_min), (T_max, muB_max)) = (binary_search_2d(ed_local, nB_local, [muQ_], INTERP, TMU_TABLES, MTP) for muQ_ in [muQ_min, muQ_max])
    (nQ_min, nQ_max) = (INTERP["nQ"](np.array([T_, muB_, muQ_]))[0] for T_, muB_, muQ_ in zip([T_min, T_max], [muB_min, muB_max], [muQ_min, muQ_max]))
    if (nQ_local < nQ_min):
        ...
        while iteration < MTP["MAXITER"]:
            ...
            T_mid, muB_mid = binary_search_2d(ed_local, nB_local, [muQ_mid], INTERP, TMU_TABLES, MTP)
            nQ_mid = INTERP["nQ"](np.array([T_mid, muB_mid, muQ_mid]))[0]
...
def invert_EOS_tables(T_ID, Ttilde, iQt, iBt, TILDE_TABLES, TMU_TABLES, INTERP, MTP, guessSol=[0.05, 0.0, 0.0]):
    mbtilde, mqtilde = TILDE_TABLES["MUB"]["Arr"][iBt], TILDE_TABLES["MUQ"]["Arr"][iQt]
    hyper_index = key(T_ID, iBt, iQt, Nb=TILDE_TABLES["MUB"]["n"], Nq=TILDE_TABLES["MUQ"]["n"])
    EN = ToEN(Ttilde, mbtilde, mqtilde)
    ed_local, nB_local, nQ_local = EN[0], EN[1], EN[2]
    success = False
    try:
        success, T_local, muB_local, muQ_local = Newton(EN, guessSol, INTERP, MTP)
    except:
        T_local, muB_local, muQ_local, muS_local = binary_search_3d(ed_local, nB_local, nQ_local, MUs, INTERP, TMU_TABLES, MTP)
        success = True
    if not success:
        T_local, muB_local, muQ_local, muS_local = binary_search_3d(EN, INTERP, TMU_TABLES, MTP)

    (P_local, s_local) = (f([T_local, muB_local, muQ_local])[0] for f in MTP["INTERP_PS"].values())
    return [T_local, muB_local, muQ_local, P_local, s_local, int(hyper_index)]
