...
def ToTilde(E, Nb): # Input GeV^4, GeV^3
...
def write_header(TILDE_TABLES):
    ...
    return [Btm, Ttm, dmubtilde, dTtilde, NBtilde, NTtilde]
...
def binary_search_2d(ed_local, nB_local, INTERP, TMU_TABLES, MTP):
    iteration = 0
    muB_min = TMU_TABLES["MUB"]["MIN"]; muB_max = TMU_TABLES["MUB"]["MAX"]
    (T_min, T_max) = (binary_search_1d(ed_local, [muB_], INTERP, TMU_TABLES, MTP) for muB_ in [muB_min, muB_max])
    (nB_min, nB_max) = (INTERP["nB"](np.array([T_, muB_]))[0] for T_, muB_ in zip([T_min, T_max], [muB_min, muB_max]))
    if (nB_local < nB_min):
...
def invert_EOS_tables(T_ID, Ttilde, iBt, TILDE_TABLES, TMU_TABLES, INTERP, MTP, guessSol=[0.05, 0.0]):
